...
plot1 <- res_c_Ea2P  %>% 
	...
	ylab("Log fitness ratio, absolute value") + xlab("Temperature") +ylim(0,1.2)
...
plot2 <- res_c_Ea2P  %>% 
	...
	ylab("Stabilization potential") + xlab("Temperature") +ylim(-0.1, 0.8)


plots <- plot1 + plot2
ggsave(filename = "figures/bayes-c_Ea2P-ss.jpeg", plot = plots, width = 10, height = 5)