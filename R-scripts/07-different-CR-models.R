...
source("R-scripts/supplemental-analysis/07S-temp-dep-macarthur-8r.R") #CR model with eight shared resources, rather than two, arrhenius-form temperature dependence
source("R-scripts/supplemental-analysis/07S-temp-dep-macarthur-unimodal.R") #unimodal temperature dependence model and CR model adapted for this
...
# Repeat main analysis with more resources - 8 resources in favour of consumer 2 - Figure S1A ####
rrc_8r <- data.frame()
for(f in 1:500){ 
...
# Repeat analysis more resources - 8 resources randomly preferred across two species - figure S1B####
rrc_8ru <- data.frame()
for(f in 1:500){ 
  hold = temp_dep_mac_8r(T = seq(10, 25, by = 0.1), 
                         ref_temp = 10,
...
# make temperature dependence unimodal - Figure S13B ##############
rrc_uni <- data.frame()
for(f in 1:500){ 
