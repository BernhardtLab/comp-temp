...
# create uni/multi columns
#isolate the ones with not enough info
mac_means_h_nas <- mac_means %>% 
  filter(is.na(heterotroph_or_autotroph)) %>% 
  dplyr::select(heterotroph_or_autotroph, general_taxon_grouping_1:species) #none, good

mac_means%>% 
  group_by(heterotroph_or_autotroph, simple_parameter) %>% 
  tally()

mac_h <- mac_means %>% 
  rename(trophy = heterotroph_or_autotroph)
...
lm_mort_h <- mac_h %>%
  filter(simple_parameter == "mortality rate") %>% 
  group_by(trophy) %>% #no unicellular estimates
  group_modify(~ {
    MCMCregress(
      activation_energy ~ 1,
      data = .x,
...
mortality_rates_h <- mac_h %>% 
  filter(simple_parameter == "mortality rate") %>% 
  dplyr::select(activation_energy, trophy)
...
growth_rates_h <- mac_h %>% 
  filter(simple_parameter == "resource growth rate") %>% 
  dplyr::select(trophy, activation_energy)
...
# consumption rate  ---------------------------------------------------
lm_consumption_rate_h <- mac_h %>%
  filter(simple_parameter == "consumption rate") %>% 
  group_by(trophy) %>% #only 1 unicellular observation
  group_modify(~ {
    MCMCregress(
      activation_energy ~ 1,
      data = .x,
...
# stitch all these dfs together for use in other scripts
# bind_rows(lm_mort_h, lm_rgr_h, lm_conv_eff_h, lm_carrying_capacity_h, lm_consumption_rate_h) %>%
# write_csv(., "data/processed-data/trophy_param_post_dists.csv")

### get summary stats for each parameter #####
#in order to prevent small changes to parameter distribution values with each simulation, I do not re-run the regressions each time.
data_h <- read_csv("data/processed-data/trophy_param_post_dists.csv") 

param_sum_h <- data_h %>%
  group_by(parameter, trophy) %>% 
  summarize(
    across(
      intercept,
      list(
...
mort_ea_plot_h <-
  ggplot() + 
  geom_density(aes(x = intercept), fill = "#00BFC4", alpha = 0.5, data = filter(data_h, parameter == "mortality_rate")) + 
  geom_point(aes(x = activation_energy, y = 0), data = mortality_rates_h, color = "#00BFC4", size = 5, alpha = 0.5) +
  geom_point(aes(x = activation_energy, y = 0), data = mortality_rates_h, color = "#252525", size = 5, shape = 1) +
  geom_vline(aes(xintercept = mean(intercept)), color = "#00BFC4", data = filter(data_h, parameter == "mortality_rate")) +
  coord_cartesian(ylim = c(0, 12), xlim = c(-1.5, 2)) +
  scale_y_continuous(breaks = c(0, 3, 6, 9, 12)) +
  labs(y = "Density", x = "Temperature Sensitivity (eV)") +
  theme_cowplot(font_size = 20) + 
...
rgr_plot_h <-
  ggplot() + 
  geom_density(aes(x = intercept, group = trophy, fill = trophy), alpha = 0.5, data = filter(data_h, parameter == "resource_growth_rate")) + 
  geom_point(aes(x = activation_energy, y = 0), data = filter(growth_rates_h, trophy == "heterotroph"), color = "#00BFC4", size = 5, alpha = 0.5) +
  geom_point(aes(x = activation_energy, y = 0), data = filter(growth_rates_h, trophy == "autotroph"), color = "pink", size = 5, alpha = 0.5) +
  geom_point(aes(x = activation_energy, y = 0), data = growth_rates_h, color = "#252525", size = 5, shape = 1) +
  geom_vline(aes(xintercept = mean(intercept)), color = "#00BFC4", data = filter(data_h, parameter == "resource_growth_rate" & trophy == "heterotroph")) +
  geom_vline(aes(xintercept = mean(intercept)), color = "darkred", data = filter(data_h, parameter == "resource_growth_rate" & trophy == "autotroph")) +
  coord_cartesian(ylim = c(0, 12), xlim = c(-1.5, 2)) +
  scale_y_continuous(breaks = c(0, 3, 6, 9, 12)) +
  labs(y = "Density", x = "Temperature Sensitivity (eV)") +
  theme_cowplot(font_size = 20) + 
...
conv_eff_plot_h <- 
  ggplot() + 
  geom_density(aes(x = intercept), fill = "#00BFC4", alpha = 0.5, data = filter(data_h, parameter == "conversion_efficiency")) + 
  geom_point(aes(x = activation_energy, y = 0), data = filter(conv_rates_h, trophy == "heterotroph"), color = "#00BFC4", size = 5, alpha = 0.5) +
  # geom_point(aes(x = activation_energy, y = 0), data = filter(conv_rates_h, trophy == "autotroph"), color = "pink", size = 5, alpha = 0.5) +
  geom_point(aes(x = activation_energy, y = 0), data = conv_rates_h, color = "#252525", size = 5, shape = 1) +
  geom_vline(aes(xintercept = mean(intercept)), color = "#00BFC4", data = filter(data_h, parameter == "conversion_efficiency" & trophy == "heterotroph")) +
  coord_cartesian(ylim = c(0, 12), xlim = c(-1.5, 2)) +
  scale_y_continuous(breaks = c(0, 3, 6, 9, 12)) +
  labs(y = "Density", x = "Temperature Sensitivity (eV)") +
  theme_cowplot(font_size = 20) + 
...
carrying_capacity_plot_h <- 
  ggplot() + 
  geom_density(aes(x = intercept, group = trophy, fill = trophy), alpha = 0.5, data = filter(data_h, parameter == "carrying_capacity")) + 
  geom_point(aes(x = activation_energy, y = 0), data = filter(carrying_capacity_h, trophy == "autotroph"), color = "pink", size = 5, alpha = 0.5) +
  geom_point(aes(x = activation_energy, y = 0), data = filter(carrying_capacity_h, trophy == "heterotroph"), color = "#00BFC4", size = 5, alpha = 0.5) +
  geom_point(aes(x = activation_energy, y = 0), data = carrying_capacity_h, color = "#252525", size = 5, shape = 1) +
  geom_vline(aes(xintercept = mean(intercept)), color = "#00BFC4", data = filter(data_h, parameter == "carrying_capacity" & trophy == "heterotroph")) +
  geom_vline(aes(xintercept = mean(intercept)), color = "darkred", data = filter(data_h, parameter == "carrying_capacity" & trophy == "autotroph")) +
  coord_cartesian(ylim = c(0, 12), xlim = c(-1.5, 2)) +
  scale_y_continuous(breaks = c(0, 3, 6, 9, 12)) +
  labs(y = "Density", x = "Temperature Sensitivity (eV)") +
  theme_cowplot(font_size = 20) + 
...
consumption_rate_plot_h <-
  ggplot() + 
  geom_density(aes(x = intercept), fill = "#00BFC4", alpha = 0.5, data = filter(data_h, parameter == "consumption rate")) + 
  geom_point(aes(x = activation_energy, y = 0), data = filter(consumption_rate_h, trophy == "heterotroph"), color = "#00BFC4", size = 5, alpha = 0.5) +
  geom_point(aes(x = activation_energy, y = 0), data = consumption_rate_h, color = "#252525", size = 5, shape = 1) +
  geom_vline(aes(xintercept = mean(intercept)), color = "#00BFC4", data = filter(data_h, parameter == "consumption rate" & trophy == "heterotroph")) +
  coord_cartesian(ylim = c(0, 12), xlim = c(-1.5, 2)) +
  scale_y_continuous(breaks = c(0, 3, 6, 9, 12)) +
  labs(y = "Density", x = "Temperature Sensitivity (eV)") +
  theme_cowplot(font_size = 20) + 
...
# plot inter-process TAs
mac_h %>% 
  group_by(simple_parameter, trophy) %>% 
  summarise(n = n())
...
interTAs_h <-
  ...
  mutate(parameter1 = str_replace_all(parameter, "_", " "),
         ...
         parameter1 = fct_reorder(parameter1, mean)) %>% 
  ggplot() + 
  geom_point(aes(x = parameter1, y = mean, group = trophy, colour = trophy, fill = trophy), size = 3) + 
  geom_errorbar(aes(x = parameter1, ymin = ci_low, ymax = ci_up, colour = trophy), width = 0.2) + 
  theme_cowplot(font_size = 20) +
  scale_x_discrete(
    labels = c(expression(italic(K)[italic(k)]),
               expression(italic(c)[italic(ik)]),
...
#### save multipanel #######
ea_plots_h <-
  consumption_rate_plot_h + rgr_plot_h + carrying_capacity_plot_h + conv_eff_plot_h + mort_ea_plot_h + interTAs_h +
  plot_annotation(tag_levels = "A") #there are no autotrophcellular mortality rate estimates, so I'm not sure why there are two dots there

# ggsave(filename = "figures/trophy-ea-plots.pdf", ea_plots_h, width = 16, height = 12)

bothsets <- ea_plots_h / ea_plots
# ggsave(filename = "figures/subset-ea-plots.pdf", bothsets, width = 18, height = 28)


#from this, it seems there is a systematic difference in the temperature dependence of growth rates between autotrophs and heterotrophs and unicellular and multicellular organisms
